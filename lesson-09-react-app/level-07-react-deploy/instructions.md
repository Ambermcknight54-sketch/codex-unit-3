...
6. Build the website to distributable files: In the terminal, run the command `npm run build`. Vite will create the `dist` folder and bundle the website code into it. Observe that there are files in the `dist` folder than in your project. Bundling them together helps them run efficiently in the browser.
7. Commit the build: The `dist` folder should be commited, but by default, `.gitignore` may block it. Remove `dist` from `.gitignore`. Commit the files in the `dist` folder.
8. Create the online repo: In the GitHub website, create a repository called `react-deploy`. Default settings are ok.
9. Push to the repo: In VS Code, add a remote. Give it the URL to the `react-deploy` repository. Set the name of the remote to `origin`.
...
11. View the deployed website in GitHub pages: Go back to `Settings` → `Pages`. Within 60 seconds, the link to your deployed website will appear. Open the link, then open Dev Tools. Observe there are file errors in the console.
12. Sync the repo: Because a commit was done through the GitHub website, pull the latest changes locally with VS Code.
13. Create the repo root path: With VS Code, in the `react-deploy/public/` folder, create the file `repoRoot.js`. Let it export a string with the name of your repo, surrounded by forward slashes. Example: `const repoRoot = "/react-deploy/"`. This is the same as the root path of your home page in GitHub. Because GitHub Pages renders repos in a subfolder, the repo root path is required.
...
20. Use a custom GitHub 404 page: With VS Code, move the file `404.html` into the `react-deploy/public/` folder. GitHub will automatically load this file for 404 errors. This file saves invalid URLs in `localStorage`, then uses `repoRoot.js` to redirect to the home page.
...
- By default, GitHub deploys `index.html` in the root of the repository. GitHub Actions is required to deploy from a different folder.
