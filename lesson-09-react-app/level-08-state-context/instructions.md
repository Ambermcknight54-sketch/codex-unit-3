...
1. NOTE: This level requires `prisma-template` and `spa-templa` from a previous level. Complete these tasks from scratch. Do not copy/paste unless the task permits it.
...
18. Create a page that uses `StateContext` in a synchronized way: In th `src/pages/` folder, create `Counter.jsx` that exports the `Counter` component which returns a `main` tag. In the `main` tag, render `StateContext`. In `App.jsx`, add a `Route` that renders `Counter`. Example: `path="/counter"`. In `Navbar.jsx`, add a link for `Counter`.
19. Add states for `Counter`: In the `src/states/` folder, create `counter.js` that exports a `Map` object. Example: `export const counter = new Map();`. Then add a state to start a count at 0. Example: `counter.add("count", 0);`. Add `counter.set("message", "Click on the counter buttons.")` In `Counter.jsx`, use the initial state. Example: `initialState={counter}`.
...
24. Create a component to decrement the count: In `src/components/`, create `Decrement.jsx` that renders a TWE button with a `-` as its label. Add `const [count, setCount] = useStateContext("count");`. Add another `useStateContext` for `"message"`. In the `button` tag, add a click handler. Example: `onClick={handleClick}`. In the function `handleClick`, decrement the count and change the message. Example: `setCount(count - 1);`. In `Counter.jsx`, before the `Count` component, render `Decrement`.
...
26. Create a component to increment the count: In `src/components/`, create `Increment.jsx` that renders a TWE button with a `+` as its label. Use `useStateContext` for `"count"` and `"message"`. Example: `const [message, setMessage] = useStateContext("message");`. In the `button` tag, add `onClick={handleClick}`. In the function `handleClick`, increment the count and change the message. Example: `setMessage("You clicked on INCREMENT");`. In `Counter.jsx`, after the `Count` component, render `Increment`.
...
28. In the `Home` component, add a `p` tag that explains how use `StateContext` and `useStateContext` to manage state without prop drilling.
...
- The setter can be ommitted if it will not be used by a component. Example: `const [myMessage] = useStateContext("myMessage");`
- Wrap `StateContext` tags around components that should have access to the states manages by `StateContext`.
